...
class RRT(object):
    ...
    def is_free_motion(self, x1: np.ndarray, x2: np.ndarray) -> bool:
        """
        ...
        Returns:
            Whether the motion from x1 to x2 is collision free.
        """
        raise NotImplementedError("is_free_motion must be overriden by a subclass of RRT")

    def nearest_neighbor(self, x: np.ndarray) -> int:
        """
    ...
    def neighbors(self, x: np.ndarray, r: float) -> list[Node]:
        """
        ...
        Returns:
            List of Nodes in self.V of nodes with radius r of the query state.
        """
        raise NotImplementedError("neighbors must be overriden by a subclass of RRT")

    def steer_towards(self, x1: np.ndarray, x2: np.ndarray, eps: float) -> np.ndarray:
        """
        ...
        Returns:
            State resulting from bounded steering
        """
    ...
    def solve(self, 
        ...
        Updates self.V and self.path if successful.
    ...
    def solve_optimal(self, 
                      eps: float, 
                      max_iters: int=1000, 
                      goal_bias: float=0.05) -> float:
        """
        Compute a path from a start point to a goal region, avoiding obstacles, 
        ...
        Returns:
            Cost-to-arrive at the goal node from the starting node, or inf if 
...
class GeometricRRT(RRT):
    ...
    def steer_towards(self, x1: np.ndarray, x2: np.ndarray, eps: float) -> np.ndarray:
        # Consult function specification in parent (RRT) class.
        ##### YOUR CODE STARTS HERE #####
        # Hint: This should take 1-4 lines.
        raise NotImplementedError("Need to implement code here.")
        ###### YOUR CODE ENDS HERE ######
